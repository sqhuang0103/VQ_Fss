...
from numpy import (array, unravel_index, nditer, linalg, random, subtract, max,
                   ...
                   nanmean, nansum)
from torch import (tensor, linalg, sub, max, pow, exp, zeros, ones, arange, ger,
                   meshgrid, matmul, logical_and, mean, std, argsort, linspace, transpose,
                   einsum, prod, sqrt, cat, diff, mul)
...
def _cov(input_vec):
    ...
    cov_m = torch.matmul(x.T, x) / (x.shape[0] - 1)
    return cov_m


def _eig(a, eigvals_only=False):
    if a.dim() != 2 or a.size(0) != a.size(1):
        raise ValueError("Input must be a square 2D tensor")
    if eigvals_only:
        return torch.eig(a, eigenvectors=False)[0]
    else:
...
def _nansum(a, dim=None):
    if dim is None:
        return torch.sum(a[torch.isnan(a)])
    else:
        return torch.sum(a, dim=dim, keepdim=True, dtype=torch.float32, where=~torch.isnan(a))

def _nanmean(a, dim=None):
    if dim is None:
        return torch.mean(a[~torch.isnan(a)])
    else:
        return _nansum(a, dim=dim) / torch.sum(~torch.isnan(a), dim=dim)
...
def _build_iteration_indexes(data_len, num_iterations,
    ...
    if random_generator:
        indices = torch.randperm(iterations.size()[0])
        iterations = iterations[indices]
...
class MiniSom(object):
    def __init__(self, x, y, input_len, sigma=1.0, learning_rate=0.5,
        ...
        # gener.manual_seed(random_seed)


        # self._random_generator = random.RandomState(random_seed)
        self._random_generator = _get_random_generator(random_seed)
        ...
        self._weights = torch.rand(x, y, input_len)* 2 - 1
    ...
    def _gaussian(self, c, sigma):
        ...
        d = 2 * sigma * sigma
        ax = exp(-pow(self._xx - self._xx.T[c], 2) / d)
        ay = exp(-pow(self._yy - self._yy.T[c], 2) / d)
        return (ax * ay).T  # the external product gives a matrix

    def _mexican_hat(self, c, sigma):
        """Mexican hat centered in c."""
        p = pow(self._xx - self._xx.T[c], 2) + pow(self._yy - self._yy.T[c], 2)
        d = 2 * sigma * sigma
    ...
    def _triangle(self, c, sigma):
        ...
        return ger(triangle_x, triangle_y)

    def _cosine_distance(self, x, w):
        num = (w * x).sum(axis=2)
        denum = mul(linalg.norm(w, axis=2), linalg.norm(x))
        return 1 - num / (denum + 1e-8)

    def _euclidean_distance(self, x, w):
        return linalg.norm(sub(x, w), axis=-1)

    def _manhattan_distance(self, x, w):
        return linalg.norm(sub(x, w), ord=1, axis=-1)

    def _chebyshev_distance(self, x, w):
        return max(sub(x, w), axis=-1)
    ...
    def winner(self, x):
        ...
        self._activate(x)
        return _unravel_index(self._activation_map.argmin(),
    ...
    def quantization(self, data):
        ...
        return self._weights[_unravel_index(winners_coords,
    ...
    def random_weights_init(self, data):
        ...
        while not it.finished:
            ...
            rand_i = torch.randint(0, len(data), (1,)).item()
    ...
    def pca_weights_init(self, data):
        ...
        if len(self._neigx) == 1 or len(self._neigy) == 1:
            ...
            warn(msg)
        pc_length, pc = _eig(_cov(transpose(data)))
    ...
    def distance_map(self, scaling='sum'):
        ...
        um = float('nan') * zeros((self._weights.shape[0],
    ...
    def _distance_from_weights(self, data):
        ...
        """
        input_data = data.clone().detach().requires_grad_(False)
        weights_flat = self._weights.reshape(-1, self._weights.shape[2])
        input_data_sq = pow(input_data, 2).sum(axis=1, keepdims=True)
        weights_flat_sq = pow(weights_flat, 2).sum(axis=1, keepdims=True)
        cross_term = matmul(input_data, weights_flat.T)
    ...
    def topographic_error(self, data):
        ...
        b2my_xy = _unravel_index(b2mu_inds, self._weights.shape[:2])
        b2mu_x, b2mu_y = b2my_xy[0], b2my_xy[1]
        dxdy = cat((diff(b2mu_x), diff(b2mu_y)), dim=1)
