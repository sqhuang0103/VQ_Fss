...
class SOProtoConv(MultiProtoAsConv):
    def __init__(self,proto_grid, feature_hw, upsample_mode = 'bilinear'):
        super(SOProtoConv, self).__init__(proto_grid, feature_hw, upsample_mode)
    ...
    def SOM_labeledV3(self,nerous,iter=10000,exclude=False, BGleft = False):
        # from minisom import MiniSom
        from .som import MiniSom
        ...
        # _y = _y.detach().cpu()
        som = MiniSom(nerous, nerous, _x.shape[-1], sigma=1.,
                      learning_rate=0.1, neighborhood_function='bubble',random_seed=1234)
        som.random_weights_init(_x)
        som.train(_x, iter, random_order=True, verbose=False)
        # import pdb
        # pdb.set_trace()
        FG_position = []
        BG_position = []
        map = torch.argmin(som._distance_from_weights(_x),dim=1)
        for map_id in range(nerous*nerous):
            map_area = torch.sum(map==map_id)
            _y_area = torch.sum(_y[map==map_id])
            ratio = _y_area/map_area
            if ratio >= 0.95:
                FG_position.append(map_id)
            else:
        ...
        if len(FG_position) > 0:
            ...
            FG_nerou = _x[_y==1][0,:]
            FG_nerou = FG_nerou.unsqueeze(0)
            # FG_nerou = np.array([FG_nerou])
        BG_nerou = som.get_weights()[np.unravel_index(BG_position,(nerous,nerous))]
        # FG_nerou = torch.FloatTensor(FG_nerou)
        # BG_nerou = torch.FloatTensor(BG_nerou)
        # map = torch.from_numpy(map)
        return BG_nerou, FG_nerou, map
